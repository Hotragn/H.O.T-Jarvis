...
- [x] `done` Reflection v2: duplicate detection now compares *meaning* where it can. Lessons are embedded on creation (and backfilled by the existing index pass), and `duplicate_match` prefers cosine over token overlap, falling back to words when either side has no vector or the widths disagree (different embedding models must never be compared). The cosine floor is much stricter than the token one because sentence embeddings put any two sentences on a shared topic at 0.7-0.85; the logged reason names which method decided, since 0.93 overlap and 0.93 cosine mean different things. Forgetting is now a soft delete (schema v5): a dropped lesson moves to a **forgotten** tab with the reason, and one click restores it. Deciding what matters to the user is a guess, so the app's guess is reversible.
...
- [x] `done` Voice v2: wake word + hands-free conversation. No second model — the energy VAD decides when someone spoke, the already-loaded Whisper transcribes it, and `core::hotword` matches the phrase tolerantly (one edit per word; "hay jarvis"/"jarvus" wake, "travis" deliberately doesn't). `core::conversation` is the tested state machine: wake → request → answer → an 8s follow-up window that needs no wake phrase, expiring back to armed. The mic closes while the assistant speaks so it can't transcribe itself; sleep phrases ("stop listening", "never mind") end a session by voice; stray noise and short takes never reach the model. `mic::listen_until_endpoint` gives automatic endpointing (bounded twice), and the header badge shows armed vs actually-capturing.
- [x] `done` Voice v3 (barge-in): you can talk over an answer and it stops and listens. The honest problem is echo — with no AEC a mic next to a speaker hears the assistant as loudly as the user, so a fixed threshold either fires on its own voice or needs a shout. `core::bargein` calibrates against the echo instead: it measures its own loudness in this room for the first half-second of playback, then only fires on sound clearly above that, sustained past a cough. Energy only, never transcription — audio is read for loudness and dropped frame by frame, so `wants_audio` excluding Speaking still holds and the assistant cannot hear itself into a request. `wants_barge_monitor` is the separate flag for that. Documented limits: headphones make it near-perfect, a loud laptop speaker can defeat it.
- [ ] `ready` Voice v3 (iOS): native SFSpeech recognition path — needs a Mac to build and test
...
- [x] `done` Auto mode v2: a background heartbeat runs cycles unattended. It wakes every few seconds and asks `may_start` one question, rather than sleeping for the whole cycle gap — a loop that sleeps 15 minutes at a time makes disarming look broken. New `min_idle_secs` gate (default 2 min) keeps unattended model calls out of a live conversation, checked last so a hard halt is reported in preference. `CycleGate` is an atomic latch so the heartbeat and the manual button can't overlap and double the caps (the cycle gap is only written when a cycle *finishes*, so `may_start` alone can't provide this). Each wake is reported as a Beat in the panel. Self-approval for the deferred set remains deliberately unimplemented.
- [x] `done` Research-to-feature loop: first scan run and logged in `docs/RESEARCH.md`. Three findings with citations, each mapped to a hero feature, stating what the paper *validates* in the current design as well as what's missing — a scan that only produces a wishlist avoids the question of whether the design is already right. Also records what was deliberately not proposed (RL-based approaches need paid compute; multi-agent memory multiplies calls into rate limits; in-weight skills need training) so the free-forever boundary stays explicit. Proposals below.
- [ ] `ready` Lesson utility measurement (from arXiv:2605.12978 — consolidated memory can drift *below* the no-memory baseline, silently). We already log which insight ids rode along in each prompt and whether the answer was graded helpful; nothing joins them. Join over the event log for a per-lesson helpfulness rate plus a no-lesson baseline, surface the comparison, and demote lessons that correlate with worse outcomes instead of waiting for decay. Needs a minimum-evidence gate: grading is sparse and voluntary.
- [ ] `ready` Cost-weighted confidence gates (from arXiv:2601.07767 — models state uncertainty adequately then fail to act on it under varying penalties). The ask threshold is one global constant, but writing a note, authoring a skill, and answering in chat are not equally costly to get wrong. Give `ActionKind` clearances a required-confidence floor scaled to how hard the action is to undo. Connects two subsystems that currently don't talk.
- [ ] `ready` Skill retirement via the existing forgetting engine (from arXiv:2605.19576 — LLM-authored skills measured +0.0pp against a no-skill baseline where human-curated gave +16.2pp; libraries drift silently without outcome gates and a size cap). A passing test proves a skill runs, not that it helps. Reuse `core::forgetting` with run outcomes in place of corroborations, keep the protection window (their ablation shows *harsh* retirement is worse than none), and make retirement a restorable deactivation rather than a delete.
